...
/* ========== Auth handlers ========== */
btnGoogle.addEventListener('click', () => loginWithProvider(googleProvider));
btnFacebook.addEventListener('click', () => loginWithProvider(fbProvider));
...
btnEmail.addEventListener('click', async () => {
    ...
    if (!email) return;
    const password = prompt("Zadej heslo (nový uživatel bude vytvořen):");
    if (!password) return;
    status.innerHTML = '<span class="loader"></span> Přihlašuji...';
    try {
        ...
        if (err.code === 'auth/user-not-found') {
            try {
                ...
                status.textContent = 'Chyba: ' + err2.message;
            }
        } else {
...
/* ========== UI logika pro typy menu a doručení ========== */
typSelect.addEventListener('change', () => {
    ...
    switch(t) {
        case 'Polévka':
            chkPolevka.checked = true;
            chkJidlo1.checked = false;
            chkJidlo2.checked = false;
            break;
        case 'Jídlo':
        ...
        case 'Půl + půl':
            chkPolevka.checked = false;
            chkJidlo1.checked = true;
            chkJidlo2.checked = true;
            break;
        default:
            break;
    }
...
function validateOrder(payload) {
    ...
    if (t === 'Celé menu' && (payload.polevka !== 'Ano' || (payload.jidlo1 !== 'Ano' && payload.jidlo2 !== 'Ano'))) return "Pro 'Celé menu' zvol polévku a alespoň jedno hlavní jídlo.";
    if (t === 'Poloviční menu') {
        if (payload.polevka !== 'Ano') return "Poloviční menu vyžaduje polévku.";
        if (!['jidlo1','jidlo2'].includes(payload.whichHalf)) return "U polovičního menu vyber, které hlavní půlku chceš.";
    }
